  ...
  function save(productArray){
	  ...
	  //alert("clientid=="+clientid+"==netotal=="+netotal+"==taxtotal=="+taxtotal+"==netaxtotal=="+netaxtotal);
	    var x=new XMLHttpRequest();
	  	...
	  	x.open("POST","saveData.jsp?products="+productArray+"&clientid="+clientid+"&netotal="+netotal+"&taxtotal="+taxtotal+"&netaxtotal="+netaxtotal,true);
	  	x.send();  
  }
  function setClient(){
		   ...
		   if (x.readyState==4 && x.status==200)
		    {
		      ...
		      items=items.split('####');
		           var  clientid=items[0];
		           var clientname=items[1];
		           var clientTax=items[2];
		         ...
		         // alert("clientname====="+clientname);
		           ...
		           optionscurr += '<option value="' + clientid[i] + '####' + clientTax[i] + '">' + clientname[i] + '</option>';
		           }
