<%@page import="com.dashboard.projectexecution.serviceupdate.ServiceUpdateDAO" %>
...
$(document).ready(function () {
    ...
    $('#jqxSerCount').on('rowdoubleclick', function (event) 
    		...
    		$("#overlay, #PleaseWait").show();
    		var id=1;
    		$("#serschedulediv").load("serScheduleDetails.jsp?clientid="+clientid+"&date="+date+"&barchval="+barchval+"&dtype="+dtype+"&id="+id+"&isprior="+isprior+"&grp="+grp+"&emp="+emp+"&mem="+mem+"&bttnview="+bttnview+"&area="+area+"&chkfromdate="+'<%=chkfromdate%>'+"&fromdate="+'<%=fromdate%>');
