...
for(int k1=0;k1<aa1.length;k1++){
	 ...
	 mainarray1.add(temp1);
	  ...
	  //  System.out.println("===ordsql====="+ordsql);
	    ...
	    String ordsql1="select  sum(amount),sum(total)total,sum(discount)dis,sum(nettotal)nettotal,sum(taxamount)nettax,sum(nettaxamount)finalamt from my_deld where rdocno in("+masterdocno+") ";
		  //  System.out.println("===ordsql====="+ordsql);
	   ...
	   int val= DAO.insert(date, refnos, "0", "1", "1", 0, clientid, masterdocno, "DEL", "0", description, prodamt, dis, nettotal, "0", "0", finalamt, "A", "INV", mainarray, mainarray2, mainarray3, session, request, masterdocno, "0", date, loc, "credit", "0", mainarray4, 0, 0.0, 0.0, 0.0, 0.0, nettax, cmbbilltype, "0", "0","0");
