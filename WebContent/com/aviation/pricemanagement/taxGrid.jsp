...
$(document).ready(function () {
	...
	var source =
	{
            ...
            $("#jqxCalcData").on('celldoubleclick', function (event){
            	var datafield=event.args.datafield;
            	var rowindextemp = event.args.rowindex;
	      	    document.getElementById("rowindex3").value = rowindextemp; 
            	if(datafield == "tax"){ 
            		taxTypeSearchContent("taxSearch.jsp?id=1");
            	}
            });
            ...
            $("#jqxCalcData").on('cellvaluechanged', function (event){
            	...
            	if(datafield=="tax"){
            		num=0;
            		$("#jqxCalcData").jqxGrid('setcellvalue', rowBoundIndex, "curr", "usc");
            	}
            	if(datafield=="percentage"){
            		var perc= $('#jqxCalcData').jqxGrid('getcellvalue', rowBoundIndex, "percentage");
            		var amount=parseFloat(price)*parseFloat(perc)/100;
            		$("#jqxCalcData").jqxGrid('setcellvalue', rowBoundIndex, "amount", amount);
            	}
            	else if(datafield=="amount"){
            		var amount= parseFloat($('#jqxCalcData').jqxGrid('getcellvalue', rowBoundIndex, "amount"));
            		var perc=(parseFloat(amount)/parseFloat(price))*100;
            		$("#jqxCalcData").jqxGrid('setcellvalue', rowBoundIndex, "percentage", perc);
            		
            		var tax= $("#jqxCalcData").jqxGrid('getcolumnaggregateddata', 'amount', ['sum'],true);
 ...
 function funSaveData(rowindex){
 	var rows = $("#jqxCalcData").jqxGrid('getrows');
  	...
  	var tax=$('#jqxCalcData').jqxGrid('getcellvalue', 0, "tax");
     		...
     		if (r){
     			...
     			var gridarray=new Array();
		  	    for(var i = 0; i < rows.length; i++){
		  	        gridarray[i]=(rows[i].taxid+"::"+rows[i].currid+"::"+$('#jqxCalcData').jqxGrid('getcelltext', i, "percentage")+"::"+rows[i].amount+"::");
		  	    }
	...
	x.onreadystatechange=function(){
		if (x.readyState==4 && x.status==200){
			...
			if(parseInt(items)=="0")  
			{
				...
				$("#jqxPriceMngt").jqxGrid('setcellvalue', rowindex1, "tax", tax);
