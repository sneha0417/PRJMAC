...
<%	
	...
	try{
		...
		sql2="select vendid,invno from my_mcpbd where tr_no="+trno+" and vendid!=0 group by vendid,invno";  
		//System.out.println("sql2vendor fetch========"+sql2);
		 ...
		 for(int j=0;j<vndarray.size();j++){   
			 ...
			 sql1="select d.qty,d.unitprice,coalesce(ac.cldocno,0) cldocno,coalesce(ac.refname,0) refname,coalesce(ac.acno,0) acno,d.costtype,d.acno glacno,d.costcode,d.tax,d.taxamt,d.description,d.total,d.amount,d.invno,d.invdate,m.date fdate from my_mcpbd d left join my_mcpbm m on m.tr_no=d.tr_no left join my_acbook ac on d.vendid=ac.cldocno and ac.dtype='VND' where d.tr_no="+trno+" and d.vendid="+vnd[0]+" and d.invno='"+vnd[1]+"'";
			 ...
			 while(rs.next()) {
			   ...
			   if(tax>0){
				...
				value=viewDAO.insert(fdate, fdate, "DIR", 0, "AP", clacno, refname, "1", "1", "", "", newdesc, session, 
						"A", vndtotal, gridarray, "CPU", request, sqlinvdate, invno,invdate, 1,  0.0, vndtotal);  
