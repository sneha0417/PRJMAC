...
$(document).ready(function () {
   ...
   var rendererstring=function (aggregates){
	 ...
	 if (cell1 != undefined && cell1.datafield == 'polcodename') {  
                 ...
                 {	 
	                  ...
	                  if (key == 9) { 
	                    ...
	                    if(document.getElementById("rreftype").value=="DLY")
		            	  {
    ...
    if ($("#mode").val() == "A" || $("#mode").val() == "E") {
    	  ...
    	  if($("#mode").val() == "A" && document.getElementById("rreftype").value=="DLY")
    		  {
    		var summaryData10= $("#prdGridID").jqxGrid('getcolumnaggregateddata', 'amount', ['sum'],true);
     	   
		   	var aa1=summaryData10.sum;
		 
		   	document.getElementById("charge").value=0;
		 	document.getElementById("discount").value=0;
		 	document.getElementById("taxamount").value=0;
    		   	document.getElementById("totalamount").value=aa1;
    		   	document.getElementById("nettotal").value=aa1;
    		   	document.getElementById("invvalue").value=aa1;
    		   	calculatedatanet();
    		  }
...
function calculatedatagrid(rowBoundIndex)
{
	...
	var brate=$('#prdGridID').jqxGrid('getcellvalue', rowBoundIndex, "bevrate");
	var prate=$('#prdGridID').jqxGrid('getcellvalue', rowBoundIndex, "polrate");
	var sqm=$('#prdGridID').jqxGrid('getcellvalue', rowBoundIndex, "sqm");
	...
	if(quantity==0)
		{
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "totsize" ,0);
		}
	else
		{
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "totsize" ,(((parseFloat(width)/1000)*(parseFloat(height)/1000))*parseFloat(quantity)));
		}
 ...
 var brates=0;
	...
	{
		brate=0;
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "bevvalue" ,0);
	}
	else
		{
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "bevvalue" ,(((parseFloat(width)+parseFloat(height))/1000)*2*parseFloat(quantity)*parseFloat(brate)));
	...
	{
		prate=0;
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "polcharge" ,0);
	}
	else
		{
		$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "polcharge",(((parseFloat(width)+parseFloat(height))/1000)*2*parseFloat(quantity)*parseFloat(prate)));
		...
		if(sqm=="YES")
		 {
			...
			$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "amount",(((parseFloat(price)*parseFloat(quantity))*parseFloat(sqlval))+(parseFloat(brates)+parseFloat(braprates))));
		 }
		else
			{
			$('#prdGridID').jqxGrid('setcellvalue', rowBoundIndex, "amount",((parseFloat(price)*parseFloat(quantity))+(parseFloat(brates)+parseFloat(braprates))));
			}
